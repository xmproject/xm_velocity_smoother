...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
        ...
        (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      double MA = sqrt(    v_inc *     v_inc +     w_inc *     w_inc +     u_inc *     u_inc);
      double MB = sqrt(max_v_inc * max_v_inc + max_w_inc * max_w_inc + max_u_inc * max_u_inc);
      ...
      double theta1 = atan2(Bv, Bu) - atan2(Av, Au);
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  dynamic_reconfigure_server = new dynamic_reconfigure::Server<xm_velocity_smoother::paramsConfig>(nh);
